...
Feat analyze(sampler2D tex, vec2 center, vec2 texSz, float cpx) {
    ...
    vec2 o[5] = vec2[](
        vec2(0.0),
        vec2(0.38, 0.0), vec2(-0.38, 0.0),
        vec2(0.0, 0.38), vec2(0.0, -0.38)
    );
...
vec3 weights(Feat f) {
    float gl = uThreshGradLow  > 0.0 ? uThreshGradLow  : 0.0004;
    float gh = uThreshGradHigh > 0.0 ? uThreshGradHigh : 0.007;
    float vl = uThreshVarLow   > 0.0 ? uThreshVarLow   : 0.0008;
    float vh = uThreshVarHigh  > 0.0 ? uThreshVarHigh  : 0.012;

    float sharp = smoothstep(gl*2.0, gh*2.5, f.grad) * smoothstep(vl, vh, f.var) * mix(0.75, 1.15, f.dirCons);
    float sm = 1.0 - smoothstep(gl, gh, f.grad);
